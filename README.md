...
| 失败 | 主会话或子 Agent 异常终止 | 子代理诊断行，或主会话最后一个 turn/end 为 error / max-tokens / interrupted / blocked / aborted |
| 已完成 | 验收通过 | 其余非空会话 |

- **任务状态与 Agent 状态分离**：卡片上的「子 Agent」统计用标记表达
  `● N 运行中 / ✓ N 已完成 / ○ N 等待 / ✕ N 失败`（绿色只给运行中圆点，已完成为
  中性灰勾，绝不用头像）。
- **失败设独立列，暂停不设列**：失败显示为卡片红色提示 `⚠ N 个 Agent 执行失败` +
  重试/查看错误；暂停是卡片操作/徽标（`⏸ 已暂停`，卡仍留在进行中列，真实执行
  `session.cancel()`，队列保留）。
- **主会话失败也算失败**：宿主列表投影不暴露主会话的 turn 错误，所以看板在读
  取已打开会话的 conversation（最后一个已关闭 turn 的 `turn/end reason`）后，
  把 `server_is_overloaded` 这类主会话 LLM/API 错误也归入「失败」列（`⚠ 主会话
  执行失败：…`），而不是因为子 Agent 全部成功就落进「已完成」。记录持久化，
  刷新后依然有效；未打开过的会话保持宿主投影状态。
...
- **看板**：待执行 / 进行中 / 需要处理 / 验收中 / 失败 / 已完成 六状态列
